function [gof,gaussfit,lnZ] = ratio_histgrams(all_blue,all_red,lnORnormal,correction,intensity_type,save_name,criterion)
...
switch lnORnormal
    ...
    case 'normal'
        ...
        xlabel('Ratio');
        ylabel('Frequency');
        title(strcat('n = ',num2str(length(ratio)),correction,intensity_type));
        saveas(gcf,strcat(save_name,'_HistoRatio_',criterion,'_',correction,'_',intensity_type,'.fig'), 'fig');
...
%         xlabel('lnZ');
%         ylabel('Frequency');
%         title(strcat('n = ',num2str(length(lnZ)),correction,intensity_type));
%         saveas(gcf,strcat(save_name,'_HistoLnZ_',criterion,'_',correction,'_',intensity_type,'.fig'), 'fig');
%         hold off
